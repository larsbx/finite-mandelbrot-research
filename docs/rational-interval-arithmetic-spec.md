...
| finite angle-measure density along a carrier | `src/C1_carrier_density_profile.mojo` (`CarrierDensityProfile`) | CONFORMS | BigZ-backed `Q` densities, residues, and per-level increments for every prefix of a residual directive carrier, each prefix measured by the kernel above (`docs/C1_separated_pair_density.md`); a level address that is not periodic, a separator the kernel refuses, a negative increment, a rising residue, and increments that do not sum to the final density all reject; the separator of each level is declared, never derived from the level's address, and no level's measure is a statement about the carrier or about any named pair |
