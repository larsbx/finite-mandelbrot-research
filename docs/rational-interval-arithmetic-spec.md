...
| 2.1–2.5 complex I_Q and Horner, checked transition | `src/checked_complex_interval.mojo` (`CheckedComplexIQResult`) | CONFORMS-CHECKED | rank-2 interval coordinate arithmetic and ascending-coefficient Horner evaluation propagate every rejected component; checked `P_{2,1}` and derivative paths are compiled; not yet used by Krawczyk acceptance |
