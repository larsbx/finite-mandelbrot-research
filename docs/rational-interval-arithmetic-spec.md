...
| 1.1 integer backend, phase one | `src/bigint_z.mojo` (`BigZ`) | DEMO | dynamic base-`10^9` limbs; exact signed construction, add/sub/mul/equality/order; exceeds `Int64` magnitude; remains non-certificate because gcd, exact division, and canonical serialization are pending |
