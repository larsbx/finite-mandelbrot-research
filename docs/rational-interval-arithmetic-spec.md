...
| 2.1–2.5 I_Q, complex boxes | `src/interval_q.mojo` (`IQ`, `ComplexIQ`) | CONFORMS | BigZ endpoints; `singleton` constructors for degenerate boxes; J1 and endpoint rejection enforced; reciprocal rejects zero-containing intervals; sign and containment/inclusion predicates preserve an explicit rejected state |
