...
| 1.1 integer backend, phase two | `src/bigint_z.mojo` (`BigZ`) | DEMO | dynamic base-`10^9` limbs; exact signed ring/order operations, quotient/remainder, rejected non-divisions, and Euclidean gcd; exceeds `Int64` magnitude; remains non-certificate because canonical serialization is pending |
