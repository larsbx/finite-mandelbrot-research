...
| Known trivial-fiber class import | `src/proof_grade_misiurewicz_trivial_fiber_classification.mojo`, `src/C1_theorem_tag_payload_instances.mojo` | checked source record and payload bound to the proof-grade `c=-2`, exact type-`(2,1)` target; wrong target/type/source/payload-kind reject | class-specific import accepted; generic MLC, all-fiber triviality, residual closure, and C1 remain false |
| Checked finite-certificate composition | `src/checked_finite_certificate_gate.mojo` | finite inputs and both theorem tags accepted; canonical incidence replay remains explicitly false | full certificate remains rejected |
